...
## P.GEN.03   定义泛型函数时，如果该函数实现用到来自 trait 定义的相关行为，需要为泛型指定相关 trait 的限定
...
## P.GEN.05  不要随便使用 `impl Trait` 语法替代泛型限定
...
## P.GEN.06   不要使用太多泛型参数和 trait 限定，否则会增长编译时间
