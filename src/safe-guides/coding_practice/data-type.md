...
- [P.TYP.01 类型转换要尽量使用安全的方式](./data-type/P.TYP.01.md)
- [P.TYP.02 对数组和集合容器进行索引要使用 `usize` 类型](./data-type/P.TYP.02.md)
- [P.TYP.03 必要时，应该使得类型可以表达更明确的语义，而不是只是直接使用原生类型](./data-type/P.TYP.03.md)
...
- [单元类型](./data-type/unit.md)
    - [G.TYP.UNT.01 当函数不关心返回值但要处理错误时应使用单元类型](./data-type/unit/G.TYP.UNT.01.md)
...
- [整数](./data-type/int.md)
    ...
    - [G.TYP.INT.02 对于大整数字面量宜使用十六进制表示](./data-type/int/G.TYP.INT.02.md)
    - [G.TYP.INT.03 避免有符号整数和无符号整数之间的强制转换](./data-type/int/G.TYP.INT.03.md)
    - [G.TYP.INT.04 对负数取模计算的时候不应使用`%`](./data-type/int/G.TYP.INT.04.md)
...
- [引用](./data-type/ref.md)
    - [P.TYP.REF.01 使用引用的时候要注意其生命周期不要重合](./data-type/ref/P.TYP.REF.01.md)
...
- [元组](./data-type/tuple.md)
    - [P.TYP.TUP.01 宜使用元组解构来同时定义多个变量](./data-type/tuple/P.TYP.TUP.01.md)
    - [G.TYP.TUP.01 使用元组时，其元素最多不宜超过3个](./data-type/tuple/G.TYP.TUP.01.md)
- [固定长度数组](./data-type/array.md)
    - [P.TYP.ARR.01 当数组长度在编译期就已经确定，应优先使用固定长度数组，而非动态数组（ Vec<T>）](./data-type/array/P.TYP.ARR.01.md)
...
- [结构体](./data-type/struct.md)
    ...
    - [P.TYP.SCT.02 当需要很多构造函数，或构造含有很多可选配置项时，宜使用构建者模式](./data-type/struct/P.TYP.SCT.02.md)
    - [P.TYP.SCT.03 结构体实例需要默认实现时，宜使用Default特质](./data-type/struct/P.TYP.SCT.03.md)
...
- [枚举体](./data-type/enum.md)
    - [P.TYP.ENM.01 需要取出 Enum 值的时候宜使用 `std::mem::take/swap/replace`](./data-type/enum/P.TYP.ENM.01.md)
